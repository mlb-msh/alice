...
a = 1.2   # contrast factor
b = 20    # brightness shift

linear_img = np.zeros_like(img, dtype=np.uint8)
for i in range(rows):
    for j in range(cols):
        new_val = int(a * img[i, j] + b)
        linear_img[i, j] = np.clip(new_val, 0, 255)

# ---------------- Logarithmic Transformation ----------------
c = 255 / np.log(1 + np.max(img))  # scaling constant

log_img = np.zeros_like(img, dtype=np.uint8)
for i in range(rows):
    for j in range(cols):
        new_val = c * np.log(1 + img[i, j])
        log_img[i, j] = np.clip(new_val, 0, 255)

# ---------------- Power-Law (Gamma) Transformation ----------------
gamma = 0.5   # <1 brightens, >1 darkens
c = 255 / (255 ** gamma)  # normalization constant

power_img = np.zeros_like(img, dtype=np.uint8)
for i in range(rows):
    for j in range(cols):
        new_val = c * (img[i, j] ** gamma)
        power_img[i, j] = np.clip(new_val, 0, 255)

# ---------------- Visualization ----------------
titles = ["Original", "Linear", "Logarithmic", "Power (Gamma)"]
images = [img, linear_img, log_img, power_img]
